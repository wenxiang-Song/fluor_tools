...
# 文件路径
data_file = './predict/input/target.csv'  # 原始数据文件
mapping_file = './predict/data/00_solvent_mapping.csv'  # solvent 与 solvent_num 的映射表
output_file = './predict/input/input.csv'  # 替换后的输出文件
...
# 读取 CSV 文件
df = pd.read_csv("./predict/input/input.csv")  # 替换为你的文件路径
...
# 保存到新的 CSV 文件
df.to_csv("./predict/input/input.csv", index=False)
print("完成分子性质预测")
...
if __name__ == "__main__":
    input_csv = './predict/input/input.csv'  # 替换为你的输入文件路径
    output_csv = './predict/input/input.csv'  # 输出文件路径
...
# 读取子结构文件
substructures_df = pd.read_csv('./predict/data/00_mmp_substructure.csv')
substructures = substructures_df['fragment'].tolist()

# 读取目标分子文件
target_df = pd.read_csv('./predict/input/input.csv')
...
# 保存结果
target_df.to_csv('./predict/input/input.csv', index=False)
print("\n处理完成！已添加所有子结构标签列。")
...
# 输入输出路径
input_file = './predict/input/input.csv'   # 请替换为你的实际文件路径
output_file = './predict/input/target_smiles_morgan.csv'
...
# 输入输出路径
input_file = './predict/input/input.csv'   # 请替换为你的实际文件路径
output_file = './predict/input/target_sol_morgan.csv'
